...
| Next | **P3-08 cocotb beta** (dep P3-03) is the only unstarted Phase-3 WP. Then Phase 4 (P4-01 IpDef contract), where the P3-06 checkers finally pay off. **v0.3.0 is prepared but NOT tagged** — this work is on branch `claude/continue-previous-p5qub7`, and tagging belongs on `main` after merge; CI also does not run on this branch (ci.yml triggers on main + PRs only) | 2-agent budget per session |
