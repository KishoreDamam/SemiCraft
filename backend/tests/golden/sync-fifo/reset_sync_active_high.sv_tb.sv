// SemiCraft v0.4.0
// Testbench: sync_fifo_tb (config hash: 10a93a7f945f)
...
module sync_fifo_tb;
    ...
    rd_data_reset_value: assert property (@(posedge clk) $fell(rst) |-> rd_data == 8'd0)
        else $fatal(1, "SVA FAIL: rd_data_reset_value");
    empty_after_reset: assert property (@(posedge clk) $fell(rst) |-> empty == 1'd1)
        else $fatal(1, "SVA FAIL: empty_after_reset");
    not_full_after_reset: assert property (@(posedge clk) $fell(rst) |-> full == 1'd0)
        else $fatal(1, "SVA FAIL: not_full_after_reset");
    count_zero_after_reset: assert property (@(posedge clk) $fell(rst) |-> count == 4'd0)
