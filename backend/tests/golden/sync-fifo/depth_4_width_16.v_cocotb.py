# SemiCraft v0.4.0
# cocotb testbench for sync_fifo (config hash: 40241d67c043)
...
@cocotb.test(timeout_time=3440, timeout_unit="ns")
async def smoke(dut):
    ...
    assert dut.full.value == 1, (
        f"SMOKE FAIL: full at cycle 5 expected 1, "
        f"got {int(dut.full.value)}"
    )
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 0, (
        f"SMOKE FAIL: empty at cycle 14 expected 0, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 1, (
        f"SMOKE FAIL: full at cycle 14 expected 1, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 4, (
        f"SMOKE FAIL: count at cycle 14 expected 4, "
        f"got {int(dut.count.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.clk)
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 0, (
        f"SMOKE FAIL: empty at cycle 16 expected 0, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 1, (
        f"SMOKE FAIL: full at cycle 16 expected 1, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 4, (
        f"SMOKE FAIL: count at cycle 16 expected 4, "
        f"got {int(dut.count.value)}"
    )
    await FallingEdge(dut.clk)
    dut.rd_en.value = 1
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 1, (
        f"SMOKE FAIL: empty at cycle 21 expected 1, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 0, (
        f"SMOKE FAIL: full at cycle 21 expected 0, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 0, (
        f"SMOKE FAIL: count at cycle 21 expected 0, "
        f"got {int(dut.count.value)}"
    )
    assert dut.rd_data.value == 65535, (
        f"SMOKE FAIL: rd_data at cycle 21 expected 65535, "
        f"got {int(dut.rd_data.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.clk)
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 1, (
        f"SMOKE FAIL: empty at cycle 23 expected 1, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 0, (
        f"SMOKE FAIL: full at cycle 23 expected 0, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 0, (
        f"SMOKE FAIL: count at cycle 23 expected 0, "
        f"got {int(dut.count.value)}"
    )
    assert dut.rd_data.value == 65535, (
        f"SMOKE FAIL: rd_data at cycle 23 expected 65535, "
        f"got {int(dut.rd_data.value)}"
    )
    for _ in range(1):
        await FallingEdge(dut.clk)
