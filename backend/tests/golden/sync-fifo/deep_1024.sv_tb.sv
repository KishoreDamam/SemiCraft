// SemiCraft v0.3.0
// Testbench: sync_fifo_tb (config hash: 042a7b1b4ddc)
...
module sync_fifo_tb;
    ...
    initial begin
        ...
        fork
            begin
                ...
                for (watchdog_i = 0; watchdog_i < 16664; watchdog_i++) @(posedge clk);
                $fatal(1, "TIMEOUT: sync_fifo_tb exceeded 16664 cycles");
            end
        join_none
        ...
        #1;
        if (empty !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: empty at cycle 1034 expected 0, got %0d", empty);
        end
        if (full !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: full at cycle 1034 expected 1, got %0d", full);
        end
        if (count !== 11'd1024) begin
            $fatal(1, "SMOKE FAIL: count at cycle 1034 expected 1024, got %0d", count);
        end
        repeat (2) @(negedge clk);
        wr_en = 1'd0;
        #1;
        if (empty !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: empty at cycle 1036 expected 0, got %0d", empty);
        end
        if (full !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: full at cycle 1036 expected 1, got %0d", full);
        end
        if (count !== 11'd1024) begin
            $fatal(1, "SMOKE FAIL: count at cycle 1036 expected 1024, got %0d", count);
        end
        @(negedge clk);
        rd_en = 1'd1;
        repeat (1024) @(negedge clk);
        #1;
        if (empty !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: empty at cycle 2061 expected 1, got %0d", empty);
        end
        if (full !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: full at cycle 2061 expected 0, got %0d", full);
        end
        if (count !== 11'd0) begin
            $fatal(1, "SMOKE FAIL: count at cycle 2061 expected 0, got %0d", count);
        end
        if (rd_data !== 16'd65535) begin
            $fatal(1, "SMOKE FAIL: rd_data at cycle 2061 expected 65535, got %0d", rd_data);
        end
        repeat (2) @(negedge clk);
        rd_en = 1'd0;
        #1;
        if (empty !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: empty at cycle 2063 expected 1, got %0d", empty);
        end
        if (full !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: full at cycle 2063 expected 0, got %0d", full);
        end
        if (count !== 11'd0) begin
            $fatal(1, "SMOKE FAIL: count at cycle 2063 expected 0, got %0d", count);
        end
        if (rd_data !== 16'd65535) begin
            $fatal(1, "SMOKE FAIL: rd_data at cycle 2063 expected 65535, got %0d", rd_data);
        end
    ...
    rd_data_reset_value: assert property (@(posedge clk) $rose(rst_n) |-> rd_data == 16'd0)
    ...
    count_zero_after_reset: assert property (@(posedge clk) $rose(rst_n) |-> count == 11'd0)
