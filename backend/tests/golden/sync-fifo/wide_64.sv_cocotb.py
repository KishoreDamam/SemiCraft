# SemiCraft v0.4.0
# cocotb testbench for sync_fifo (config hash: 9c7becf10bc8)
...
@cocotb.test(timeout_time=5360, timeout_unit="ns")
async def smoke(dut):
    ...
    assert dut.count.value == 4, (
        f"SMOKE FAIL: count at cycle 5 expected 4, "
        f"got {int(dut.count.value)}"
    )
    await FallingEdge(dut.clk)
    await Timer(1, units="ns")
    ...
    assert dut.count.value == 3, (
        f"SMOKE FAIL: count at cycle 6 expected 3, "
        f"got {int(dut.count.value)}"
    )
    await FallingEdge(dut.clk)
    await Timer(1, units="ns")
    ...
    assert dut.count.value == 2, (
        f"SMOKE FAIL: count at cycle 7 expected 2, "
        f"got {int(dut.count.value)}"
    )
    await FallingEdge(dut.clk)
    await Timer(1, units="ns")
    ...
    assert dut.count.value == 1, (
        f"SMOKE FAIL: count at cycle 8 expected 1, "
        f"got {int(dut.count.value)}"
    )
    await FallingEdge(dut.clk)
    await Timer(1, units="ns")
    ...
    assert dut.count.value == 0, (
        f"SMOKE FAIL: count at cycle 9 expected 0, "
        f"got {int(dut.count.value)}"
    )
    await FallingEdge(dut.clk)
    dut.rd_en.value = 0
    dut.wr_data.value = 18446744073709551615
    dut.wr_en.value = 1
    for _ in range(16):
        await FallingEdge(dut.clk)
    await Timer(1, units="ns")
    assert dut.empty.value == 0, (
        f"SMOKE FAIL: empty at cycle 26 expected 0, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 1, (
        f"SMOKE FAIL: full at cycle 26 expected 1, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 16, (
        f"SMOKE FAIL: count at cycle 26 expected 16, "
        f"got {int(dut.count.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.clk)
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 0, (
        f"SMOKE FAIL: empty at cycle 28 expected 0, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 1, (
        f"SMOKE FAIL: full at cycle 28 expected 1, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 16, (
        f"SMOKE FAIL: count at cycle 28 expected 16, "
        f"got {int(dut.count.value)}"
    )
    await FallingEdge(dut.clk)
    dut.rd_en.value = 1
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 1, (
        f"SMOKE FAIL: empty at cycle 45 expected 1, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 0, (
        f"SMOKE FAIL: full at cycle 45 expected 0, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 0, (
        f"SMOKE FAIL: count at cycle 45 expected 0, "
        f"got {int(dut.count.value)}"
    )
    assert dut.rd_data.value == 18446744073709551615, (
        f"SMOKE FAIL: rd_data at cycle 45 expected 18446744073709551615, "
        f"got {int(dut.rd_data.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.clk)
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 1, (
        f"SMOKE FAIL: empty at cycle 47 expected 1, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 0, (
        f"SMOKE FAIL: full at cycle 47 expected 0, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 0, (
        f"SMOKE FAIL: count at cycle 47 expected 0, "
        f"got {int(dut.count.value)}"
    )
    assert dut.rd_data.value == 18446744073709551615, (
        f"SMOKE FAIL: rd_data at cycle 47 expected 18446744073709551615, "
        f"got {int(dut.rd_data.value)}"
    )
    for _ in range(1):
        await FallingEdge(dut.clk)
