// SemiCraft v0.3.0
// Testbench: sync_fifo_tb (config hash: 9c7becf10bc8)
...
module sync_fifo_tb;
    ...
    initial begin
        ...
        fork
            begin
                ...
                for (watchdog_i = 0; watchdog_i < 536; watchdog_i++) @(posedge clk);
                $fatal(1, "TIMEOUT: sync_fifo_tb exceeded 536 cycles");
            end
        join_none
        ...
        @(negedge clk);
        rd_en = 1'd0;
        wr_data = 64'd18446744073709551615;
        wr_en = 1'd1;
        repeat (16) @(negedge clk);
        #1;
        if (empty !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: empty at cycle 26 expected 0, got %0d", empty);
        end
        if (full !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: full at cycle 26 expected 1, got %0d", full);
        end
        if (count !== 5'd16) begin
            $fatal(1, "SMOKE FAIL: count at cycle 26 expected 16, got %0d", count);
        end
        repeat (2) @(negedge clk);
        wr_en = 1'd0;
        #1;
        if (empty !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: empty at cycle 28 expected 0, got %0d", empty);
        end
        if (full !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: full at cycle 28 expected 1, got %0d", full);
        end
        if (count !== 5'd16) begin
            $fatal(1, "SMOKE FAIL: count at cycle 28 expected 16, got %0d", count);
        end
        @(negedge clk);
        rd_en = 1'd1;
        repeat (16) @(negedge clk);
        #1;
        if (empty !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: empty at cycle 45 expected 1, got %0d", empty);
        end
        if (full !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: full at cycle 45 expected 0, got %0d", full);
        end
        if (count !== 5'd0) begin
            $fatal(1, "SMOKE FAIL: count at cycle 45 expected 0, got %0d", count);
        end
        if (rd_data !== 64'd18446744073709551615) begin
            $fatal(1, "SMOKE FAIL: rd_data at cycle 45 expected 18446744073709551615, got %0d", rd_data);
        end
        repeat (2) @(negedge clk);
        rd_en = 1'd0;
        #1;
        if (empty !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: empty at cycle 47 expected 1, got %0d", empty);
        end
        if (full !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: full at cycle 47 expected 0, got %0d", full);
        end
        if (count !== 5'd0) begin
            $fatal(1, "SMOKE FAIL: count at cycle 47 expected 0, got %0d", count);
        end
        if (rd_data !== 64'd18446744073709551615) begin
            $fatal(1, "SMOKE FAIL: rd_data at cycle 47 expected 18446744073709551615, got %0d", rd_data);
        end
    ...
    rd_data_reset_value: assert property (@(posedge clk) $rose(rst_n) |-> rd_data == 64'd0)
    ...
    count_zero_after_reset: assert property (@(posedge clk) $rose(rst_n) |-> count == 5'd0)
