# SemiCraft v0.3.0
# cocotb testbench for sync_fifo (config hash: 8a111a739d58)
...
@cocotb.test(timeout_time=166640, timeout_unit="ns")
async def smoke(dut):
    ...
    await FallingEdge(dut.clk)
    dut.rd_en.value = 0
    dut.wr_data.value = 65535
    dut.wr_en.value = 1
    for _ in range(1024):
        await FallingEdge(dut.clk)
    await Timer(1, units="ns")
    assert dut.empty.value == 0, (
        f"SMOKE FAIL: empty at cycle 1034 expected 0, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 1, (
        f"SMOKE FAIL: full at cycle 1034 expected 1, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 1024, (
        f"SMOKE FAIL: count at cycle 1034 expected 1024, "
        f"got {int(dut.count.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.clk)
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 0, (
        f"SMOKE FAIL: empty at cycle 1036 expected 0, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 1, (
        f"SMOKE FAIL: full at cycle 1036 expected 1, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 1024, (
        f"SMOKE FAIL: count at cycle 1036 expected 1024, "
        f"got {int(dut.count.value)}"
    )
    await FallingEdge(dut.clk)
    dut.rd_en.value = 1
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 1, (
        f"SMOKE FAIL: empty at cycle 2061 expected 1, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 0, (
        f"SMOKE FAIL: full at cycle 2061 expected 0, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 0, (
        f"SMOKE FAIL: count at cycle 2061 expected 0, "
        f"got {int(dut.count.value)}"
    )
    assert dut.rd_data.value == 65535, (
        f"SMOKE FAIL: rd_data at cycle 2061 expected 65535, "
        f"got {int(dut.rd_data.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.clk)
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 1, (
        f"SMOKE FAIL: empty at cycle 2063 expected 1, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 0, (
        f"SMOKE FAIL: full at cycle 2063 expected 0, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 0, (
        f"SMOKE FAIL: count at cycle 2063 expected 0, "
        f"got {int(dut.count.value)}"
    )
    assert dut.rd_data.value == 65535, (
        f"SMOKE FAIL: rd_data at cycle 2063 expected 65535, "
        f"got {int(dut.rd_data.value)}"
    )
    for _ in range(1):
        await FallingEdge(dut.clk)
