# SemiCraft v0.3.0
# cocotb testbench for sync_fifo (config hash: 0bb6f2d964fc)
...
async def smoke(dut):
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 0, (
        f"SMOKE FAIL: empty at cycle 18 expected 0, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 1, (
        f"SMOKE FAIL: full at cycle 18 expected 1, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 8, (
        f"SMOKE FAIL: count at cycle 18 expected 8, "
        f"got {int(dut.count.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.clk)
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 0, (
        f"SMOKE FAIL: empty at cycle 20 expected 0, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 1, (
        f"SMOKE FAIL: full at cycle 20 expected 1, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 8, (
        f"SMOKE FAIL: count at cycle 20 expected 8, "
        f"got {int(dut.count.value)}"
    )
    await FallingEdge(dut.clk)
    dut.rd_en.value = 1
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 1, (
        f"SMOKE FAIL: empty at cycle 29 expected 1, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 0, (
        f"SMOKE FAIL: full at cycle 29 expected 0, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 0, (
        f"SMOKE FAIL: count at cycle 29 expected 0, "
        f"got {int(dut.count.value)}"
    )
    assert dut.rd_data.value == 255, (
        f"SMOKE FAIL: rd_data at cycle 29 expected 255, "
        f"got {int(dut.rd_data.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.clk)
    ...
    await Timer(1, units="ns")
    assert dut.empty.value == 1, (
        f"SMOKE FAIL: empty at cycle 31 expected 1, "
        f"got {int(dut.empty.value)}"
    )
    assert dut.full.value == 0, (
        f"SMOKE FAIL: full at cycle 31 expected 0, "
        f"got {int(dut.full.value)}"
    )
    assert dut.count.value == 0, (
        f"SMOKE FAIL: count at cycle 31 expected 0, "
        f"got {int(dut.count.value)}"
    )
    assert dut.rd_data.value == 255, (
        f"SMOKE FAIL: rd_data at cycle 31 expected 255, "
        f"got {int(dut.rd_data.value)}"
    )
    for _ in range(1):
        await FallingEdge(dut.clk)
