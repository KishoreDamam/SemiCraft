# SemiCraft v0.3.0
# cocotb testbench for debouncer (config hash: 3c18a68caccf)
...
async def smoke(dut):
    ...
    assert dut.q.value == 1, (
        f"SMOKE FAIL: q at cycle 4 expected 1, got {int(dut.q.value)}"
    )
    await FallingEdge(dut.clk)
    dut.d_in.value = 0
    await FallingEdge(dut.clk)
    ...
    await Timer(1, units="ns")
    assert dut.q.value == 1, (
        f"SMOKE FAIL: q at cycle 7 expected 1, got {int(dut.q.value)}"
    )
