// SemiCraft v0.4.0
// Testbench: axil_gpio_tb (config hash: 116e98671763)
...
module axil_gpio_tb;
    ...
    initial begin
        ...
        if (rdata !== 32'd90) begin
            $fatal(1, "SMOKE FAIL: rdata at cycle 15 expected 90, got %0d", rdata);
        end
        if (rresp !== 2'd0) begin
        ...
        if (gpio_oe !== 8'd255) begin
            $fatal(1, "SMOKE FAIL: gpio_oe at cycle 18 expected 255, got %0d", gpio_oe);
        end
        @(negedge aclk);
        #1;
        if (bvalid !== 1'd0) begin
