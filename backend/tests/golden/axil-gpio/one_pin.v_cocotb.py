# SemiCraft v0.4.0
# cocotb testbench for axil_gpio (config hash: 0d3b07e1a0df)
...
async def smoke(dut):
    ...
    assert dut.gpio_oe.value == 1, (
        f"SMOKE FAIL: gpio_oe at cycle 3 expected 1, "
        f"got {int(dut.gpio_oe.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 4
    ...
    assert dut.gpio_out.value == 1, (
        f"SMOKE FAIL: gpio_out at cycle 6 expected 1, "
        f"got {int(dut.gpio_out.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 0
    ...
    assert dut.rdata.value == 0, (
        f"SMOKE FAIL: rdata at cycle 15 expected 0, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
    ...
    assert dut.gpio_oe.value == 1, (
        f"SMOKE FAIL: gpio_oe at cycle 18 expected 1, "
        f"got {int(dut.gpio_oe.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
