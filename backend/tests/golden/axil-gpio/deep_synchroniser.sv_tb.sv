// SemiCraft v0.3.0
// Testbench: axil_gpio_tb (config hash: 0cdcdef66c1f)
...
module axil_gpio_tb;
    ...
    initial begin
        ...
        fork
            begin
                ...
                for (watchdog_i = 0; watchdog_i < 320; watchdog_i++) @(posedge aclk);
                $fatal(1, "TIMEOUT: axil_gpio_tb exceeded 320 cycles");
            end
        join_none
        ...
        if (gpio_oe !== 8'd255) begin
            $fatal(1, "SMOKE FAIL: gpio_oe at cycle 3 expected 255, got %0d", gpio_oe);
        end
        @(negedge aclk);
        awaddr = 4'd4;
        awvalid = 1'd1;
        ...
        if (gpio_out !== 8'd165) begin
            $fatal(1, "SMOKE FAIL: gpio_out at cycle 6 expected 165, got %0d", gpio_out);
        end
        @(negedge aclk);
        araddr = 4'd0;
        arvalid = 1'd1;
        ...
        if (rdata !== 32'd255) begin
            $fatal(1, "SMOKE FAIL: rdata at cycle 8 expected 255, got %0d", rdata);
        end
        if (rresp !== 2'd0) begin
        ...
        if (rdata !== 32'd90) begin
            $fatal(1, "SMOKE FAIL: rdata at cycle 17 expected 90, got %0d", rdata);
        end
        if (rresp !== 2'd0) begin
        ...
        if (bresp !== 2'd2) begin
            $fatal(1, "SMOKE FAIL: bresp at cycle 20 expected 2, got %0d", bresp);
        end
        if (gpio_oe !== 8'd255) begin
            $fatal(1, "SMOKE FAIL: gpio_oe at cycle 20 expected 255, got %0d", gpio_oe);
        end
        @(negedge aclk);
        #1;
        if (bvalid !== 1'd0) begin
    ...
    pins_not_driven_after_reset: assert property (@(posedge aclk) $rose(areset_n) |-> gpio_oe == 8'd0)
