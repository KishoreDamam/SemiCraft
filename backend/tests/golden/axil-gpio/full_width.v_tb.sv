// SemiCraft v0.4.0
// Testbench: axil_gpio_tb (config hash: 65f7f1db0ad7)
...
module axil_gpio_tb;
    ...
    initial begin
        ...
        fork
            begin
                ...
                $fatal(1, "TIMEOUT: axil_gpio_tb exceeded 272 cycles");
            end
        join_none
        ...
        if (gpio_oe !== 32'd4294967295) begin
            $fatal(1, "SMOKE FAIL: gpio_oe at cycle 3 expected 4294967295, got %0d", gpio_oe);
        end
        @(negedge aclk);
        awaddr = 4'd4;
        awvalid = 1'd1;
        bready = 1'd1;
        wdata = 32'd2779096485;
        wstrb = 4'd15;
        wvalid = 1'd1;
        #1;
        if (bvalid !== 1'd0) begin
        ...
        if (gpio_out !== 32'd2779096485) begin
            $fatal(1, "SMOKE FAIL: gpio_out at cycle 6 expected 2779096485, got %0d", gpio_out);
        end
        @(negedge aclk);
        araddr = 4'd0;
        arvalid = 1'd1;
        ...
        if (rdata !== 32'd4294967295) begin
            $fatal(1, "SMOKE FAIL: rdata at cycle 8 expected 4294967295, got %0d", rdata);
        end
        if (rresp !== 2'd0) begin
            $fatal(1, "SMOKE FAIL: rresp at cycle 8 expected 0, got %0d", rresp);
        end
        @(negedge aclk);
        gpio_in = 32'd1515870810;
        @(negedge aclk);
        araddr = 4'd8;
        arvalid = 1'd1;
        rready = 1'd1;
        ...
        if (rdata !== 32'd1515870810) begin
            $fatal(1, "SMOKE FAIL: rdata at cycle 15 expected 1515870810, got %0d", rdata);
        end
        if (rresp !== 2'd0) begin
    ...
    pins_not_driven_after_reset: assert property (@(posedge aclk) $rose(areset_n) |-> gpio_oe == 32'd0)
