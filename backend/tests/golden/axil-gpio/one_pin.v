// SemiCraft v0.4.0
// Snippet: axil_gpio (config hash: 0d3b07e1a0df)
...
module axil_gpio (
    ...
    input  wire        gpio_in,    // Asynchronous pin inputs
    output wire        gpio_out,   // Pin output values
    output wire        gpio_oe     // Pin output enables (1 = drive)
);

    reg dir_value;  // DIR.value (RW)
    reg out_value;  // OUT.value (RW)
    wire in_value;  // IN.value (RO) — sampled on read
    ...
    reg in_sync0;  // Input synchroniser stage 0
    reg in_sync1;  // Input synchroniser stage 1
