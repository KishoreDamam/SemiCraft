// SemiCraft v0.4.0
// Testbench: axil_gpio_tb (config hash: 64c7ba5cccd4)
...
module axil_gpio_tb;
    ...
    initial begin
        ...
        fork
            begin
                ...
                for (watchdog_i = 0; watchdog_i < 312; watchdog_i++) @(posedge aclk);
                $fatal(1, "TIMEOUT: axil_gpio_tb exceeded 312 cycles");
            end
        join_none
        ...
        if (gpio_oe !== 24'd16777215) begin
            $fatal(1, "SMOKE FAIL: gpio_oe at cycle 3 expected 16777215, got %0d", gpio_oe);
        end
        @(negedge aclk);
        awaddr = 4'd4;
        awvalid = 1'd1;
        ...
        if (gpio_out !== 24'd10855845) begin
            $fatal(1, "SMOKE FAIL: gpio_out at cycle 6 expected 10855845, got %0d", gpio_out);
        end
        @(negedge aclk);
        araddr = 4'd0;
        arvalid = 1'd1;
        ...
        if (rdata !== 32'd16777215) begin
            $fatal(1, "SMOKE FAIL: rdata at cycle 8 expected 16777215, got %0d", rdata);
        end
        if (rresp !== 2'd0) begin
        ...
        #1;
        if (rvalid !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: rvalid at cycle 16 expected 1, got %0d", rvalid);
        end
        if (rdata !== 32'd5921370) begin
            $fatal(1, "SMOKE FAIL: rdata at cycle 16 expected 5921370, got %0d", rdata);
        end
        if (rresp !== 2'd0) begin
            $fatal(1, "SMOKE FAIL: rresp at cycle 16 expected 0, got %0d", rresp);
        end
        @(negedge aclk);
        awaddr = 4'd0;
        awvalid = 1'd1;
        ...
        #1;
        if (bvalid !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: bvalid at cycle 19 expected 1, got %0d", bvalid);
        end
        if (bresp !== 2'd2) begin
            $fatal(1, "SMOKE FAIL: bresp at cycle 19 expected 2, got %0d", bresp);
        end
        if (gpio_oe !== 24'd16777215) begin
            $fatal(1, "SMOKE FAIL: gpio_oe at cycle 19 expected 16777215, got %0d", gpio_oe);
        end
        @(negedge aclk);
        #1;
        if (bvalid !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: bvalid at cycle 20 expected 0, got %0d", bvalid);
        end
    ...
    pins_not_driven_after_reset: assert property (@(posedge aclk) $rose(areset_n) |-> gpio_oe == 24'd0)
