// SemiCraft v0.4.0
// Snippet: axil_gpio (config hash: 64c7ba5cccd4)
// AXI4-Lite GPIO, 24 pin(s)
//
...
module axil_gpio (
    ...
    input  wire [23:0] gpio_in,    // Asynchronous pin inputs
    output wire [23:0] gpio_out,   // Pin output values
    output wire [23:0] gpio_oe     // Pin output enables (1 = drive)
);

    reg [23:0] dir_value;  // DIR.value (RW)
    reg [23:0] out_value;  // OUT.value (RW)
    wire [23:0] in_value;  // IN.value (RO) — sampled on read
    ...
    assign wr_rsvd_mask = ((wr_addr_dir ? 32'hFF000000 : 32'b0) | (wr_addr_out ? 32'hFF000000 : 32'b0)) | (wr_addr_in ? 32'hFFFFFFFF : 32'b0);
    ...
    always @(posedge aclk) begin
        if (!areset_n) begin
            ...
            if (wr_sel_dir) begin
                dir_value <= (wdata_q[23:0] & wmask[23:0]) | (dir_value & (~wmask[23:0]));
            end
            if (wr_sel_out) begin
                out_value <= (wdata_q[23:0] & wmask[23:0]) | (out_value & (~wmask[23:0]));
            end
            if (arvalid && arready) begin
                rvalid <= 1'b1;
                case (araddr)
                    4'h0: begin
                        rdata <= {8'b0, dir_value};
                        rresp <= 2'b0;
                    end
                    4'h4: begin
                        rdata <= {8'b0, out_value};
                        rresp <= 2'b0;
                    end
                    4'h8: begin
                        rdata <= {8'b0, in_value};
    ...
    reg [23:0] in_sync0;  // Input synchroniser stage 0
    reg [23:0] in_sync1;  // Input synchroniser stage 1
    reg [23:0] in_sync2;  // Input synchroniser stage 2
