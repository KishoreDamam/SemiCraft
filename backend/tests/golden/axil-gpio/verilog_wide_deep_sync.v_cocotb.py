# SemiCraft v0.4.0
# cocotb testbench for axil_gpio (config hash: 64c7ba5cccd4)
...
@cocotb.test(timeout_time=3120, timeout_unit="ns")
async def smoke(dut):
    ...
    assert dut.gpio_oe.value == 16777215, (
        f"SMOKE FAIL: gpio_oe at cycle 3 expected 16777215, "
        f"got {int(dut.gpio_oe.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 4
    ...
    assert dut.gpio_out.value == 10855845, (
        f"SMOKE FAIL: gpio_out at cycle 6 expected 10855845, "
        f"got {int(dut.gpio_out.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 0
    ...
    assert dut.rdata.value == 16777215, (
        f"SMOKE FAIL: rdata at cycle 8 expected 16777215, "
    ...
    await Timer(1, units="ns")
    assert dut.rvalid.value == 1, (
        f"SMOKE FAIL: rvalid at cycle 16 expected 1, "
        f"got {int(dut.rvalid.value)}"
    )
    assert dut.rdata.value == 5921370, (
        f"SMOKE FAIL: rdata at cycle 16 expected 5921370, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
        f"SMOKE FAIL: rresp at cycle 16 expected 0, "
        f"got {int(dut.rresp.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 0
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 1, (
        f"SMOKE FAIL: bvalid at cycle 19 expected 1, "
        f"got {int(dut.bvalid.value)}"
    )
    assert dut.bresp.value == 2, (
        f"SMOKE FAIL: bresp at cycle 19 expected 2, "
        f"got {int(dut.bresp.value)}"
    )
    assert dut.gpio_oe.value == 16777215, (
        f"SMOKE FAIL: gpio_oe at cycle 19 expected 16777215, "
        f"got {int(dut.gpio_oe.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.bvalid.value == 0, (
        f"SMOKE FAIL: bvalid at cycle 20 expected 0, "
        f"got {int(dut.bvalid.value)}"
    )
