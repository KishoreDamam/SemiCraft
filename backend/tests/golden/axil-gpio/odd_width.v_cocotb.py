# SemiCraft v0.4.0
# cocotb testbench for axil_gpio (config hash: 06b21227b805)
...
async def smoke(dut):
    ...
    assert dut.gpio_oe.value == 8191, (
        f"SMOKE FAIL: gpio_oe at cycle 3 expected 8191, "
        f"got {int(dut.gpio_oe.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 4
    ...
    assert dut.gpio_out.value == 1445, (
        f"SMOKE FAIL: gpio_out at cycle 6 expected 1445, "
        f"got {int(dut.gpio_out.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 0
    ...
    assert dut.rdata.value == 8191, (
        f"SMOKE FAIL: rdata at cycle 8 expected 8191, "
    ...
    assert dut.rdata.value == 6746, (
        f"SMOKE FAIL: rdata at cycle 15 expected 6746, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 1, (
        f"SMOKE FAIL: bvalid at cycle 18 expected 1, "
        f"got {int(dut.bvalid.value)}"
    )
    assert dut.bresp.value == 2, (
        f"SMOKE FAIL: bresp at cycle 18 expected 2, "
        f"got {int(dut.bresp.value)}"
    )
    assert dut.gpio_oe.value == 8191, (
        f"SMOKE FAIL: gpio_oe at cycle 18 expected 8191, "
        f"got {int(dut.gpio_oe.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.bvalid.value == 0, (
        f"SMOKE FAIL: bvalid at cycle 19 expected 0, "
        f"got {int(dut.bvalid.value)}"
    )
