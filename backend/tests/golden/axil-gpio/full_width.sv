// SemiCraft v0.3.0
// Snippet: axil_gpio (config hash: b90e27f8c418)
// AXI4-Lite GPIO, 32 pin(s)
//
...
module axil_gpio (
    ...
    input  logic [31:0] gpio_in,    // Asynchronous pin inputs
    output logic [31:0] gpio_out,   // Pin output values
    output logic [31:0] gpio_oe     // Pin output enables (1 = drive)
);

    logic [31:0] dir_value;  // DIR.value (RW)
    logic [31:0] out_value;  // OUT.value (RW)
    logic [31:0] in_value;  // IN.value (RO) — sampled on read
    ...
    assign wr_rsvd_mask = ((wr_addr_dir ? 32'h0 : 32'b0) | (wr_addr_out ? 32'h0 : 32'b0)) | (wr_addr_in ? 32'hFFFFFFFF : 32'b0);
    ...
    always_ff @(posedge aclk) begin
        if (!areset_n) begin
            ...
            if (wr_sel_dir) begin
                dir_value <= (wdata_q[31:0] & wmask[31:0]) | (dir_value & (~wmask[31:0]));
            end
            if (wr_sel_out) begin
                out_value <= (wdata_q[31:0] & wmask[31:0]) | (out_value & (~wmask[31:0]));
            end
    ...
    logic [31:0] in_sync0;  // Input synchroniser stage 0
    logic [31:0] in_sync1;  // Input synchroniser stage 1
