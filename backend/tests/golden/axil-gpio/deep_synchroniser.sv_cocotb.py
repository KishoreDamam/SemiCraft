# SemiCraft v0.3.0
# cocotb testbench for axil_gpio (config hash: 0cdcdef66c1f)
...
@cocotb.test(timeout_time=3200, timeout_unit="ns")
async def smoke(dut):
    ...
    assert dut.rdata.value == 90, (
        f"SMOKE FAIL: rdata at cycle 17 expected 90, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
    ...
    assert dut.bresp.value == 2, (
        f"SMOKE FAIL: bresp at cycle 20 expected 2, "
        f"got {int(dut.bresp.value)}"
    )
    assert dut.gpio_oe.value == 255, (
        f"SMOKE FAIL: gpio_oe at cycle 20 expected 255, "
        f"got {int(dut.gpio_oe.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
