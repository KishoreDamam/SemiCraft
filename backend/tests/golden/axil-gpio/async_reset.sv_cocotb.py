# SemiCraft v0.4.0
# cocotb testbench for axil_gpio (config hash: 116e98671763)
...
async def smoke(dut):
    ...
    assert dut.gpio_oe.value == 255, (
        f"SMOKE FAIL: gpio_oe at cycle 3 expected 255, "
        f"got {int(dut.gpio_oe.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 4
    ...
    assert dut.gpio_out.value == 165, (
        f"SMOKE FAIL: gpio_out at cycle 6 expected 165, "
        f"got {int(dut.gpio_out.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 0
    ...
    assert dut.rdata.value == 255, (
        f"SMOKE FAIL: rdata at cycle 8 expected 255, "
    ...
    assert dut.rdata.value == 90, (
        f"SMOKE FAIL: rdata at cycle 15 expected 90, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
    ...
    assert dut.gpio_oe.value == 255, (
        f"SMOKE FAIL: gpio_oe at cycle 18 expected 255, "
        f"got {int(dut.gpio_oe.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
