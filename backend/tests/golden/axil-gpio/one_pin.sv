// SemiCraft v0.4.0
// Snippet: axil_gpio (config hash: b28d1b69442d)
// AXI4-Lite GPIO, 1 pin(s)
//
...
module axil_gpio (
    ...
    input  logic        gpio_in,    // Asynchronous pin inputs
    output logic        gpio_out,   // Pin output values
    output logic        gpio_oe     // Pin output enables (1 = drive)
);

    logic dir_value;  // DIR.value (RW)
    logic out_value;  // OUT.value (RW)
    logic in_value;  // IN.value (RO) — sampled on read
    ...
    assign wr_rsvd_mask = ((wr_addr_dir ? 32'hFFFFFFFE : 32'b0) | (wr_addr_out ? 32'hFFFFFFFE : 32'b0)) | (wr_addr_in ? 32'hFFFFFFFF : 32'b0);
    ...
    always_ff @(posedge aclk) begin
        if (!areset_n) begin
            ...
            if (wr_sel_dir) begin
                dir_value <= (wdata_q[0] & wmask[0]) | (dir_value & (~wmask[0]));
            end
            if (wr_sel_out) begin
                out_value <= (wdata_q[0] & wmask[0]) | (out_value & (~wmask[0]));
            end
            if (arvalid && arready) begin
                rvalid <= 1'b1;
                case (araddr)
                    4'h0: begin
                        rdata <= {31'b0, dir_value};
                        rresp <= 2'b0;
                    end
                    4'h4: begin
                        rdata <= {31'b0, out_value};
                        rresp <= 2'b0;
                    end
                    4'h8: begin
                        rdata <= {31'b0, in_value};
                        rresp <= 2'b0;
                    end
                    default: begin
                        rdata <= 32'b0;
    ...
    logic in_sync0;  // Input synchroniser stage 0
    logic in_sync1;  // Input synchroniser stage 1
