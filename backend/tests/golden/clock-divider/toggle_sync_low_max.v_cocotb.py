# SemiCraft v0.3.0
# cocotb testbench for clock_divider (config hash: 6d1b0e8c7c5b)
...
@cocotb.test(timeout_time=5244320, timeout_unit="ns")
async def smoke(dut):
    ...
    assert dut.clk_out.value == 0, (
        f"SMOKE FAIL: clk_out at cycle 0 expected 0, "
    ...
    assert dut.clk_out.value == 1, (
        f"SMOKE FAIL: clk_out at cycle 32768 expected 1, "
        f"got {int(dut.clk_out.value)}"
    )
