# SemiCraft v0.4.0
# cocotb testbench for clock_divider (config hash: f93ac723f3c3)
...
async def smoke(dut):
    ...
    await Timer(1, units="ns")
    assert dut.p_clkOut.value == 0, (
        f"SMOKE FAIL: p_clkOut at cycle 0 expected 0, "
        f"got {int(dut.p_clkOut.value)}"
    )
    await FallingEdge(dut.p_clk)
    await Timer(1, units="ns")
    assert dut.p_clkOut.value == 1, (
        f"SMOKE FAIL: p_clkOut at cycle 1 expected 1, "
        f"got {int(dut.p_clkOut.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.p_clk)
