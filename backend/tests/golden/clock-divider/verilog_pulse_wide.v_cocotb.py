# SemiCraft v0.3.0
# cocotb testbench for clock_divider (config hash: eb16677d0086)
...
@cocotb.test(timeout_time=9520, timeout_unit="ns")
async def smoke(dut):
    ...
    assert dut.clk_out.value == 1, (
        f"SMOKE FAIL: clk_out at cycle 100 expected 1, got {int(dut.clk_out.value)}"
    )
