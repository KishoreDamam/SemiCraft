# SemiCraft v0.4.0
# cocotb testbench for edge_detector (config hash: 6c652ad52515)
...
async def smoke(dut):
    ...
    dut.p_d.value = 1
    await Timer(1, units="ns")
    assert dut.p_pulse.value == 1, (
        f"SMOKE FAIL: p_pulse at cycle 2 expected 1, "
        f"got {int(dut.p_pulse.value)}"
    )
    await FallingEdge(dut.p_clk)
    dut.p_d.value = 0
    await Timer(1, units="ns")
    assert dut.p_pulse.value == 0, (
        f"SMOKE FAIL: p_pulse at cycle 3 expected 0, "
        f"got {int(dut.p_pulse.value)}"
    )
    await FallingEdge(dut.p_clk)
    dut.p_d.value = 0
