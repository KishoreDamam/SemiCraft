// SemiCraft v0.1.0
// Testbench: edge_detector_tb (config hash: 614e85730eb0)
...
module edge_detector_tb;
    ...
    initial begin
        ...
        #1;
        if (pulse !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: pulse at cycle 1 expected 1, got %0d", pulse);
        end
        @(negedge clk);
        d = 1'd1;
        #1;
        if (pulse !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: pulse at cycle 2 expected 0, got %0d", pulse);
        end
        @(negedge clk);
        d = 1'd0;
        @(negedge clk);
