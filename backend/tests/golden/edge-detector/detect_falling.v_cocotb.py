# SemiCraft v0.3.0
# cocotb testbench for edge_detector (config hash: 0124aaf1e68a)
...
async def smoke(dut):
    ...
    assert dut.pulse.value == 1, (
        f"SMOKE FAIL: pulse at cycle 4 expected 1, got {int(dut.pulse.value)}"
    )
    await FallingEdge(dut.clk)
    dut.d.value = 1
    await Timer(1, units="ns")
    assert dut.pulse.value == 0, (
        f"SMOKE FAIL: pulse at cycle 5 expected 0, got {int(dut.pulse.value)}"
    )
