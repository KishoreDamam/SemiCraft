...
module axil_i2c (
    ...
    input  logic        scl_in,     // Level sensed on SCL (low while a slave stretches)
    input  logic        sda_in,     // Level sensed on SDA
    output logic        scl_oe,     // Pull SCL low; the line is never driven high
    output logic        sda_oe      // Pull SDA low; the line is never driven high
);

    logic status_busy;  // STATUS.busy (RO) — sampled on read
    logic status_done;  // STATUS.done (W1C) — current value
    logic status_done_set;  // STATUS.done (W1C) — set request; a set beats a same-cycle clear
    logic status_ack_err;  // STATUS.ack_err (W1C) — current value
    logic status_ack_err_set;  // STATUS.ack_err (W1C) — set request; a set beats a same-cycle clear
    ...
    logic cmd_start;  // CMD.start (WO) — reads back as 0
    logic cmd_write;  // CMD.write (WO) — reads back as 0
    logic cmd_read;  // CMD.read (WO) — reads back as 0
    logic cmd_ack;  // CMD.ack (WO) — reads back as 0
    logic cmd_stop;  // CMD.stop (WO) — reads back as 0
    ...
    logic wr_addr_cmd;  // Write address selects CMD
    ...
    logic wr_sel_cmd;  // Write updates CMD
    ...
    logic scl_sync0;  // SCL synchroniser
    logic scl_sync1;  // SCL synchroniser
    logic scl_sync2;  // SCL synchroniser
    logic scl_sync3;  // SCL synchroniser
    logic sda_sync0;  // SDA synchroniser
    logic sda_sync1;  // SDA synchroniser
    logic sda_sync2;  // SDA synchroniser
    logic sda_sync3;  // SDA synchroniser
    logic [2:0] state;  // 0 IDLE, 1 START, 2 BIT, 3 ACK, 4 STOP, 5 DONE
    logic [1:0] phase;  // Quarter of the bit period, 0..3
    logic [15:0] q_cnt;  // aclk cycles within one quarter
    logic [2:0] bit_cnt;  // Bits transferred so far
    logic [7:0] tx_shift;  // Outgoing byte, empties from the MSB
    logic [7:0] rx_shift;  // Incoming byte, fills from the LSB
    logic cmd_go;  // CMD write strobe, delayed one cycle
    logic scl_sensed;  // Synchronised SCL
    logic sda_sensed;  // Synchronised SDA
    ...
    assign scl_sensed = scl_sync3;

    assign sda_sensed = sda_sync3;
