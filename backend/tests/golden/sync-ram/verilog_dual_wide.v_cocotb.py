# SemiCraft v0.3.0
# cocotb testbench for sync_ram (config hash: a20114503dbf)
...
async def smoke(dut):
    ...
    dut.re.value = 0
    await Timer(1, units="ns")
    assert dut.dout.value == 4294967278, (
        f"SMOKE FAIL: dout at cycle 9 expected 4294967278, "
        f"got {int(dut.dout.value)}"
    )
    await FallingEdge(dut.clk)
    await Timer(1, units="ns")
    assert dut.dout.value == 4294967278, (
        f"SMOKE FAIL: dout at cycle 10 expected 4294967278, "
        f"got {int(dut.dout.value)}"
    )
    for _ in range(1):
        await FallingEdge(dut.clk)
