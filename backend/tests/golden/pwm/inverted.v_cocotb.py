# SemiCraft v0.3.0
# cocotb testbench for pwm (config hash: be3c5ccd374e)
...
async def smoke(dut):
    ...
    assert dut.pwm_out.value == 1, (
        f"SMOKE FAIL: pwm_out at cycle 0 expected 1, got {int(dut.pwm_out.value)}"
    )
    await FallingEdge(dut.clk)
    dut.duty.value = 128
    await Timer(1, units="ns")
    assert dut.pwm_out.value == 0, (
        f"SMOKE FAIL: pwm_out at cycle 1 expected 0, got {int(dut.pwm_out.value)}"
    )
    await FallingEdge(dut.clk)
    dut.duty.value = 128
    await FallingEdge(dut.clk)
