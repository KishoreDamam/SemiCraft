# SemiCraft v0.3.0
# cocotb testbench for axil_spi (config hash: 2a8200edaa51)
...
@cocotb.test(timeout_time=4400, timeout_unit="ns")
async def smoke(dut):
    ...
    assert dut.sclk.value == 1, (
        f"SMOKE FAIL: sclk at cycle 8 expected 1, "
        f"got {int(dut.sclk.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.sclk.value == 0, (
        f"SMOKE FAIL: sclk at cycle 9 expected 0, "
        f"got {int(dut.sclk.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.mosi.value == 1, (
        f"SMOKE FAIL: mosi at cycle 10 expected 1, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.mosi.value == 0, (
        f"SMOKE FAIL: mosi at cycle 12 expected 0, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.mosi.value == 0, (
        f"SMOKE FAIL: mosi at cycle 14 expected 0, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.mosi.value == 1, (
        f"SMOKE FAIL: mosi at cycle 16 expected 1, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.mosi.value == 1, (
        f"SMOKE FAIL: mosi at cycle 20 expected 1, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.sclk.value == 0, (
        f"SMOKE FAIL: sclk at cycle 23 expected 0, "
        f"got {int(dut.sclk.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 0
    ...
    assert dut.rdata.value == 2, (
        f"SMOKE FAIL: rdata at cycle 25 expected 2, "
    ...
    assert dut.rdata.value == 255, (
        f"SMOKE FAIL: rdata at cycle 27 expected 255, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
    ...
    assert dut.cs_n.value == 1, (
        f"SMOKE FAIL: cs_n at cycle 35 expected 1, "
        f"got {int(dut.cs_n.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
