// SemiCraft v0.4.0
// Snippet: axil_spi (config hash: 819733549f6b)
// AXI4-Lite SPI master, mode 11
//
...
module axil_spi (
    ...
    output logic        sclk,       // Serial clock, idles 1
    ...
    logic [6:0] rx_shift;  // Receive shift register, fills from the LSB
            ...
            if (!spi_active) begin
                ...
                if (half_cnt == (clkdiv_div - 16'b1)) begin
                    ...
                    if (edge_cnt == 5'd15) begin
                        spi_active <= 1'b0;
                        rxdata_data <= {rx_shift, miso_sync1};
                        status_rx_valid_set <= 1'b1;
                    end else begin
                        if (edge_cnt[0] == 1'b1) begin
                            rx_shift <= {rx_shift[5:0], miso_sync1};
                        end else begin
                            mosi <= tx_shift[7];
