// SemiCraft v0.3.0
// Testbench: axil_spi_tb (config hash: 4d1116ae1206)
...
module axil_spi_tb;
    ...
    initial begin
        ...
        #1;
        if (sclk !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: sclk at cycle 11 expected 1, got %0d", sclk);
        end
        @(negedge aclk);
        #1;
        if (mosi !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: mosi at cycle 12 expected 1, got %0d", mosi);
        end
        ...
        #1;
        if (mosi !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: mosi at cycle 16 expected 0, got %0d", mosi);
        end
        repeat (2) @(negedge aclk);
        miso = 1'd0;
        repeat (2) @(negedge aclk);
        #1;
        if (mosi !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: mosi at cycle 20 expected 0, got %0d", mosi);
        end
        repeat (2) @(negedge aclk);
        miso = 1'd1;
        repeat (2) @(negedge aclk);
        #1;
        if (mosi !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: mosi at cycle 24 expected 1, got %0d", mosi);
        end
        repeat (2) @(negedge aclk);
        miso = 1'd1;
        repeat (2) @(negedge aclk);
        #1;
        if (mosi !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: mosi at cycle 28 expected 0, got %0d", mosi);
        end
        repeat (2) @(negedge aclk);
        miso = 1'd0;
        repeat (2) @(negedge aclk);
        #1;
        if (mosi !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: mosi at cycle 32 expected 1, got %0d", mosi);
        end
        repeat (2) @(negedge aclk);
        miso = 1'd1;
        repeat (2) @(negedge aclk);
        #1;
        if (mosi !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: mosi at cycle 36 expected 1, got %0d", mosi);
        end
        repeat (3) @(negedge aclk);
        #1;
        if (sclk !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: sclk at cycle 39 expected 1, got %0d", sclk);
        end
        @(negedge aclk);
        araddr = 5'd0;
        arvalid = 1'd1;
