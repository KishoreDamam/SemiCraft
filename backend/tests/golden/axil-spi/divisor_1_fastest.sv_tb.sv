// SemiCraft v0.4.0
// Testbench: axil_spi_tb (config hash: 2a8200edaa51)
    ...
    initial begin
        ...
        fork
            begin
                ...
                for (watchdog_i = 0; watchdog_i < 440; watchdog_i++) @(posedge aclk);
                $fatal(1, "TIMEOUT: axil_spi_tb exceeded 440 cycles");
            end
        join_none
        ...
        if (sclk !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: sclk at cycle 8 expected 1, got %0d", sclk);
        end
        @(negedge aclk);
        #1;
        if (sclk !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: sclk at cycle 9 expected 0, got %0d", sclk);
        end
        @(negedge aclk);
        #1;
        if (mosi !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: mosi at cycle 10 expected 1, got %0d", mosi);
        end
        repeat (2) @(negedge aclk);
        #1;
        if (mosi !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: mosi at cycle 12 expected 0, got %0d", mosi);
        end
        repeat (2) @(negedge aclk);
        #1;
        if (mosi !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: mosi at cycle 14 expected 0, got %0d", mosi);
        end
        repeat (2) @(negedge aclk);
        #1;
        if (mosi !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: mosi at cycle 16 expected 1, got %0d", mosi);
        end
        repeat (2) @(negedge aclk);
        #1;
        if (mosi !== 1'd0) begin
        ...
        #1;
        if (mosi !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: mosi at cycle 20 expected 1, got %0d", mosi);
        end
        repeat (2) @(negedge aclk);
        #1;
        if (mosi !== 1'd1) begin
        ...
        #1;
        if (sclk !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: sclk at cycle 23 expected 0, got %0d", sclk);
        end
        @(negedge aclk);
        araddr = 5'd0;
        arvalid = 1'd1;
        ...
        if (rdata !== 32'd2) begin
            $fatal(1, "SMOKE FAIL: rdata at cycle 25 expected 2, got %0d", rdata);
        end
        if (rresp !== 2'd0) begin
        ...
        if (rdata !== 32'd255) begin
            $fatal(1, "SMOKE FAIL: rdata at cycle 27 expected 255, got %0d", rdata);
        end
        if (rresp !== 2'd0) begin
        ...
        if (cs_n !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: cs_n at cycle 35 expected 1, got %0d", cs_n);
        end
        @(negedge aclk);
        #1;
        if (bvalid !== 1'd0) begin
