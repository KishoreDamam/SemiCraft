# SemiCraft v0.4.0
# cocotb testbench for axil_spi (config hash: 32ff198b7a0a)
...
@cocotb.test(timeout_time=12080, timeout_unit="ns")
async def smoke(dut):
    ...
    assert dut.sclk.value == 1, (
        f"SMOKE FAIL: sclk at cycle 14 expected 1, "
        f"got {int(dut.sclk.value)}"
    )
    for _ in range(7):
        await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.sclk.value == 0, (
        f"SMOKE FAIL: sclk at cycle 21 expected 0, "
        f"got {int(dut.sclk.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.mosi.value == 1, (
        f"SMOKE FAIL: mosi at cycle 22 expected 1, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(3):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.mosi.value == 0, (
        f"SMOKE FAIL: mosi at cycle 36 expected 0, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(3):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.mosi.value == 0, (
        f"SMOKE FAIL: mosi at cycle 50 expected 0, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(3):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.mosi.value == 1, (
        f"SMOKE FAIL: mosi at cycle 64 expected 1, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(3):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.mosi.value == 1, (
        f"SMOKE FAIL: mosi at cycle 92 expected 1, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(3):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.mosi.value == 1, (
        f"SMOKE FAIL: mosi at cycle 106 expected 1, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(3):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.sclk.value == 0, (
        f"SMOKE FAIL: sclk at cycle 119 expected 0, "
        f"got {int(dut.sclk.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 0
    ...
    assert dut.rvalid.value == 1, (
        f"SMOKE FAIL: rvalid at cycle 121 expected 1, "
        f"got {int(dut.rvalid.value)}"
    )
    assert dut.rdata.value == 2, (
        f"SMOKE FAIL: rdata at cycle 121 expected 2, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
        f"SMOKE FAIL: rresp at cycle 121 expected 0, "
    ...
    await Timer(1, units="ns")
    assert dut.rvalid.value == 1, (
        f"SMOKE FAIL: rvalid at cycle 123 expected 1, "
        f"got {int(dut.rvalid.value)}"
    )
    assert dut.rdata.value == 45, (
        f"SMOKE FAIL: rdata at cycle 123 expected 45, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
        f"SMOKE FAIL: rresp at cycle 123 expected 0, "
        f"got {int(dut.rresp.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 0
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 1, (
        f"SMOKE FAIL: bvalid at cycle 126 expected 1, "
        f"got {int(dut.bvalid.value)}"
    )
    assert dut.bresp.value == 0, (
        f"SMOKE FAIL: bresp at cycle 126 expected 0, "
        f"got {int(dut.bresp.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 0
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 0, (
        f"SMOKE FAIL: bvalid at cycle 127 expected 0, "
        f"got {int(dut.bvalid.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.arvalid.value = 0
    await Timer(1, units="ns")
    assert dut.rvalid.value == 1, (
        f"SMOKE FAIL: rvalid at cycle 128 expected 1, "
        f"got {int(dut.rvalid.value)}"
    )
    assert dut.rdata.value == 0, (
        f"SMOKE FAIL: rdata at cycle 128 expected 0, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
        f"SMOKE FAIL: rresp at cycle 128 expected 0, "
        f"got {int(dut.rresp.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 4
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 1, (
        f"SMOKE FAIL: bvalid at cycle 131 expected 1, "
        f"got {int(dut.bvalid.value)}"
    )
    assert dut.bresp.value == 0, (
        f"SMOKE FAIL: bresp at cycle 131 expected 0, "
        f"got {int(dut.bresp.value)}"
    )
    assert dut.cs_n.value == 1, (
        f"SMOKE FAIL: cs_n at cycle 131 expected 1, "
        f"got {int(dut.cs_n.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.bvalid.value == 0, (
        f"SMOKE FAIL: bvalid at cycle 132 expected 0, "
        f"got {int(dut.bvalid.value)}"
    )
