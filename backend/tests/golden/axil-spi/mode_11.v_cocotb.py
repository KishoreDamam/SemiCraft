# SemiCraft v0.4.0
# cocotb testbench for axil_spi (config hash: bd75068ae041)
...
async def smoke(dut):
    ...
    assert dut.mosi.value == 0, (
        f"SMOKE FAIL: mosi at cycle 10 expected 0, "
        f"got {int(dut.mosi.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.sclk.value == 1, (
        f"SMOKE FAIL: sclk at cycle 11 expected 1, "
        f"got {int(dut.sclk.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.miso.value = 0
    ...
    assert dut.mosi.value == 1, (
        f"SMOKE FAIL: mosi at cycle 14 expected 1, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.mosi.value == 0, (
        f"SMOKE FAIL: mosi at cycle 22 expected 0, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.mosi.value == 1, (
        f"SMOKE FAIL: mosi at cycle 26 expected 1, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.mosi.value == 0, (
        f"SMOKE FAIL: mosi at cycle 30 expected 0, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.mosi.value == 1, (
        f"SMOKE FAIL: mosi at cycle 34 expected 1, "
        f"got {int(dut.mosi.value)}"
    )
    for _ in range(2):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.sclk.value == 1, (
        f"SMOKE FAIL: sclk at cycle 39 expected 1, "
        f"got {int(dut.sclk.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 0
    ...
    assert dut.rdata.value == 45, (
        f"SMOKE FAIL: rdata at cycle 43 expected 45, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
