# SemiCraft v0.3.0
# cocotb testbench for axil_timer (config hash: 52d0075fe3d3)
...
async def smoke(dut):
    ...
    assert dut.irq.value == 0, (
        f"SMOKE FAIL: irq at cycle 0 expected 0, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 12
    dut.awvalid.value = 1
    ...
    assert dut.irq.value == 0, (
        f"SMOKE FAIL: irq at cycle 3 expected 0, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 4
    ...
    assert dut.irq.value == 0, (
        f"SMOKE FAIL: irq at cycle 6 expected 0, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 8
    ...
    assert dut.rdata.value == 3, (
        f"SMOKE FAIL: rdata at cycle 8 expected 3, "
    ...
    assert dut.irq.value == 0, (
        f"SMOKE FAIL: irq at cycle 11 expected 0, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    ...
    await Timer(1, units="ns")
    assert dut.irq.value == 0, (
        f"SMOKE FAIL: irq at cycle 19 expected 0, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.irq.value == 1, (
        f"SMOKE FAIL: irq at cycle 20 expected 1, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 16
    ...
    assert dut.rdata.value == 1, (
        f"SMOKE FAIL: rdata at cycle 22 expected 1, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 1, (
        f"SMOKE FAIL: bvalid at cycle 25 expected 1, "
        f"got {int(dut.bvalid.value)}"
    )
    assert dut.bresp.value == 0, (
        f"SMOKE FAIL: bresp at cycle 25 expected 0, "
        f"got {int(dut.bresp.value)}"
    )
    assert dut.irq.value == 0, (
        f"SMOKE FAIL: irq at cycle 25 expected 0, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 16
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 0, (
        f"SMOKE FAIL: bvalid at cycle 26 expected 0, "
        f"got {int(dut.bvalid.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awvalid.value = 0
    ...
    assert dut.irq.value == 0, (
        f"SMOKE FAIL: irq at cycle 28 expected 0, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 16
    ...
    assert dut.irq.value == 0, (
        f"SMOKE FAIL: irq at cycle 33 expected 0, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    ...
    await Timer(1, units="ns")
    assert dut.irq.value == 0, (
        f"SMOKE FAIL: irq at cycle 41 expected 0, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.irq.value == 1, (
        f"SMOKE FAIL: irq at cycle 42 expected 1, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 16
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 1, (
        f"SMOKE FAIL: bvalid at cycle 45 expected 1, "
        f"got {int(dut.bvalid.value)}"
    )
    assert dut.bresp.value == 0, (
        f"SMOKE FAIL: bresp at cycle 45 expected 0, "
        f"got {int(dut.bresp.value)}"
    )
    assert dut.irq.value == 0, (
        f"SMOKE FAIL: irq at cycle 45 expected 0, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.bvalid.value == 0, (
        f"SMOKE FAIL: bvalid at cycle 46 expected 0, "
        f"got {int(dut.bvalid.value)}"
    )
    for _ in range(24):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.rvalid.value == 1, (
        f"SMOKE FAIL: rvalid at cycle 71 expected 1, "
        f"got {int(dut.rvalid.value)}"
    )
    assert dut.rdata.value == 0, (
        f"SMOKE FAIL: rdata at cycle 71 expected 0, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
        f"SMOKE FAIL: rresp at cycle 71 expected 0, "
        f"got {int(dut.rresp.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 16
    ...
    await Timer(1, units="ns")
    assert dut.rvalid.value == 1, (
        f"SMOKE FAIL: rvalid at cycle 73 expected 1, "
        f"got {int(dut.rvalid.value)}"
    )
    assert dut.rdata.value == 0, (
        f"SMOKE FAIL: rdata at cycle 73 expected 0, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
        f"SMOKE FAIL: rresp at cycle 73 expected 0, "
        f"got {int(dut.rresp.value)}"
    )
    assert dut.irq.value == 0, (
        f"SMOKE FAIL: irq at cycle 73 expected 0, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 0
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 1, (
        f"SMOKE FAIL: bvalid at cycle 76 expected 1, "
        f"got {int(dut.bvalid.value)}"
    )
    assert dut.bresp.value == 2, (
        f"SMOKE FAIL: bresp at cycle 76 expected 2, "
        f"got {int(dut.bresp.value)}"
    )
    assert dut.irq.value == 0, (
        f"SMOKE FAIL: irq at cycle 76 expected 0, "
        f"got {int(dut.irq.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.bvalid.value == 0, (
        f"SMOKE FAIL: bvalid at cycle 77 expected 0, "
        f"got {int(dut.bvalid.value)}"
    )
