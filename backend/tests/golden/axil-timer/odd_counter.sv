// SemiCraft v0.4.0
// Snippet: axil_timer (config hash: 2f7511403468)
// AXI4-Lite timer, 20-bit counter, 16-bit prescaler
//
...
module axil_timer (
    ...
    logic [19:0] reload_value;  // RELOAD.value (RW)
    logic [19:0] count_value;  // COUNT.value (RO) — sampled on read
    logic [15:0] prescale_value;  // PRESCALE.value (RW)
    ...
    assign wr_rsvd_mask = ((((wr_addr_ctrl ? 32'hFFFFFFF8 : 32'b0) | (wr_addr_reload ? 32'hFFF00000 : 32'b0)) | (wr_addr_count ? 32'hFFFFFFFF : 32'b0)) | (wr_addr_prescale ? 32'hFFFF0000 : 32'b0)) | (wr_addr_status ? 32'hFFFFFFFE : 32'b0);
    ...
    always_ff @(posedge aclk) begin
        if (!areset_n) begin
            ...
            if (wr_sel_reload) begin
                reload_value <= (wdata_q[19:0] & wmask[19:0]) | (reload_value & (~wmask[19:0]));
            end
            ...
            if (arvalid && arready) begin
                ...
                case (araddr)
                    ...
                    5'h4: begin
                        rdata <= {12'b0, reload_value};
                        rresp <= 2'b0;
                    end
                    5'h8: begin
                        rdata <= {12'b0, count_value};
                        rresp <= 2'b0;
                    end
                    5'hC: begin
    ...
    logic [15:0] pre_cnt;  // Prescaler counter
