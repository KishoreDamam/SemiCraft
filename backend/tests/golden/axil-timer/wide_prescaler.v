// SemiCraft v0.4.0
// Snippet: axil_timer (config hash: ec2eb3d1bf1b)
// AXI4-Lite timer, 32-bit counter, 32-bit prescaler
//
...
module axil_timer (
    ...
    reg [31:0] reload_value;  // RELOAD.value (RW)
    reg [31:0] count_value;  // COUNT.value (RO) — sampled on read
    reg [31:0] prescale_value;  // PRESCALE.value (RW)
    ...
    assign wr_rsvd_mask = ((((wr_addr_ctrl ? 32'hFFFFFFF8 : 32'b0) | (wr_addr_reload ? 32'h0 : 32'b0)) | (wr_addr_count ? 32'hFFFFFFFF : 32'b0)) | (wr_addr_prescale ? 32'h0 : 32'b0)) | (wr_addr_status ? 32'hFFFFFFFE : 32'b0);
    ...
    always @(posedge aclk) begin
        if (!areset_n) begin
            ...
            if (wr_sel_prescale) begin
                prescale_value <= (wdata_q[31:0] & wmask[31:0]) | (prescale_value & (~wmask[31:0]));
            end
    ...
    reg [31:0] pre_cnt;  // Prescaler counter
