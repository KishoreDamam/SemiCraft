...
module axil_timer (
    ...
    reg [15:0] reload_value;  // RELOAD.value (RW)
    reg [15:0] count_value;  // COUNT.value (RO) — sampled on read
    reg [7:0] prescale_value;  // PRESCALE.value (RW)
    ...
    assign wr_rsvd_mask = ((((wr_addr_ctrl ? 32'hFFFFFFF8 : 32'b0) | (wr_addr_reload ? 32'hFFFF0000 : 32'b0)) | (wr_addr_count ? 32'hFFFFFFFF : 32'b0)) | (wr_addr_prescale ? 32'hFFFFFF00 : 32'b0)) | (wr_addr_status ? 32'hFFFFFFFE : 32'b0);
    ...
    always @(posedge aclk) begin
        if (!areset_n) begin
            ...
            if (wr_sel_reload) begin
                reload_value <= (wdata_q[15:0] & wmask[15:0]) | (reload_value & (~wmask[15:0]));
            end
            if (wr_sel_prescale) begin
                prescale_value <= (wdata_q[7:0] & wmask[7:0]) | (prescale_value & (~wmask[7:0]));
            end
            ...
            if (arvalid && arready) begin
                ...
                case (araddr)
                    ...
                    5'h4: begin
                        rdata <= {16'b0, reload_value};
                        rresp <= 2'b0;
                    end
                    5'h8: begin
                        rdata <= {16'b0, count_value};
                        rresp <= 2'b0;
                    end
                    5'hC: begin
                        rdata <= {24'b0, prescale_value};
                        rresp <= 2'b0;
                    end
                    5'h10: begin
    ...
    reg [7:0] pre_cnt;  // Prescaler counter
                ...
                if (running) begin
                    if (pre_cnt == prescale_value) begin
                        ...
                        if (count_value == 16'b0) begin
                            ...
                            count_value <= count_value - 16'b1;
                        end
                    end else begin
                        pre_cnt <= pre_cnt + 8'b1;
                    end
                end
            end
        end
