// SemiCraft v0.4.0
// Snippet: axil_intc (config hash: f2ebca9f537b)
// AXI4-Lite interrupt controller, 13 source(s), edge-triggered
//
...
module axil_intc (
    ...
    input  logic [12:0] irq_in,     // Asynchronous interrupt requests
    output logic        irq_out     // Masked request; high while any enabled source pends
);

    logic [12:0] pending_value;  // PENDING.value (W1C) — current value
    logic [12:0] pending_value_set;  // PENDING.value (W1C) — set request; a set beats a same-cycle clear
    logic [12:0] enable_value;  // ENABLE.value (RW)
    logic [12:0] status_value;  // STATUS.value (RO) — sampled on read
    ...
    assign wr_rsvd_mask = ((wr_addr_pending ? 32'hFFFFE000 : 32'b0) | (wr_addr_enable ? 32'hFFFFE000 : 32'b0)) | (wr_addr_status ? 32'hFFFFFFFF : 32'b0);
    ...
    always_ff @(posedge aclk) begin
        if (!areset_n) begin
            ...
            if (wr_sel_pending) begin
                pending_value <= (pending_value & (~(wdata_q[12:0] & wmask[12:0]))) | pending_value_set;
            end else begin
                pending_value <= pending_value | pending_value_set;
            end
            if (wr_sel_enable) begin
                enable_value <= (wdata_q[12:0] & wmask[12:0]) | (enable_value & (~wmask[12:0]));
            end
            if (arvalid && arready) begin
                rvalid <= 1'b1;
                case (araddr)
                    4'h0: begin
                        rdata <= {19'b0, pending_value};
                        rresp <= 2'b0;
                    end
                    4'h4: begin
                        rdata <= {19'b0, enable_value};
                        rresp <= 2'b0;
                    end
                    4'h8: begin
                        rdata <= {19'b0, status_value};
                        rresp <= 2'b0;
                    end
                    default: begin
                        rdata <= 32'b0;
    ...
    logic [12:0] irq_sync0;  // Request synchroniser stage 0
    logic [12:0] irq_sync1;  // Request synchroniser stage 1
    logic [12:0] irq_hist;  // Previous settled request, for edge detection
