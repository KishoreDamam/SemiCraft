// SemiCraft v0.4.0
// Snippet: axil_intc (config hash: 8e1d06e3538f)
...
module axil_intc (
    ...
    input  wire [31:0] irq_in,     // Asynchronous interrupt requests
    output wire        irq_out     // Masked request; high while any enabled source pends
);

    reg [31:0] pending_value;  // PENDING.value (W1C) — current value
    wire [31:0] pending_value_set;  // PENDING.value (W1C) — set request; a set beats a same-cycle clear
    reg [31:0] enable_value;  // ENABLE.value (RW)
    wire [31:0] status_value;  // STATUS.value (RO) — sampled on read
    ...
    assign wr_rsvd_mask = ((wr_addr_pending ? 32'h0 : 32'b0) | (wr_addr_enable ? 32'h0 : 32'b0)) | (wr_addr_status ? 32'hFFFFFFFF : 32'b0);
    ...
    always @(posedge aclk) begin
        if (!areset_n) begin
            ...
            if (wr_sel_pending) begin
                pending_value <= (pending_value & (~(wdata_q[31:0] & wmask[31:0]))) | pending_value_set;
            end else begin
                pending_value <= pending_value | pending_value_set;
            end
            if (wr_sel_enable) begin
                enable_value <= (wdata_q[31:0] & wmask[31:0]) | (enable_value & (~wmask[31:0]));
            end
    ...
    reg [31:0] irq_sync0;  // Request synchroniser stage 0
    reg [31:0] irq_sync1;  // Request synchroniser stage 1
    reg [31:0] irq_hist;  // Previous settled request, for edge detection
