...
module axil_intc (
    ...
    input  logic [31:0] irq_in,     // Asynchronous interrupt requests
    output logic        irq_out     // Masked request; high while any enabled source pends
);

    logic [31:0] pending_value;  // PENDING.value (W1C) — current value
    logic [31:0] pending_value_set;  // PENDING.value (W1C) — set request; a set beats a same-cycle clear
    logic [31:0] enable_value;  // ENABLE.value (RW)
    logic [31:0] status_value;  // STATUS.value (RO) — sampled on read
    ...
    logic [31:0] irq_sync0;  // Request synchroniser stage 0
    logic [31:0] irq_sync1;  // Request synchroniser stage 1
    logic [31:0] irq_hist;  // Previous settled request, for edge detection
