// SemiCraft v0.4.0
// Snippet: axil_intc (config hash: 46f98a14ee2f)
...
module axil_intc (
    ...
    input  logic [7:0]  irq_in,     // Asynchronous interrupt requests
    output logic        irq_out     // Masked request; high while any enabled source pends
);

    logic [7:0] pending_value;  // PENDING.value (W1C) — current value
    logic [7:0] pending_value_set;  // PENDING.value (W1C) — set request; a set beats a same-cycle clear
    logic [7:0] enable_value;  // ENABLE.value (RW)
    logic [7:0] status_value;  // STATUS.value (RO) — sampled on read
    ...
    assign wr_rsvd_mask = ((wr_addr_pending ? 32'hFFFFFF00 : 32'b0) | (wr_addr_enable ? 32'hFFFFFF00 : 32'b0)) | (wr_addr_status ? 32'hFFFFFFFF : 32'b0);
    ...
    always_ff @(posedge aclk) begin
        if (!areset_n) begin
            ...
            if (wr_sel_pending) begin
                pending_value <= (pending_value & (~(wdata_q[7:0] & wmask[7:0]))) | pending_value_set;
            end else begin
                pending_value <= pending_value | pending_value_set;
            end
            if (wr_sel_enable) begin
                enable_value <= (wdata_q[7:0] & wmask[7:0]) | (enable_value & (~wmask[7:0]));
            end
            if (arvalid && arready) begin
                rvalid <= 1'b1;
                case (araddr)
                    4'h0: begin
                        rdata <= {24'b0, pending_value};
                        rresp <= 2'b0;
                    end
                    4'h4: begin
                        rdata <= {24'b0, enable_value};
                        rresp <= 2'b0;
                    end
                    4'h8: begin
                        rdata <= {24'b0, status_value};
                        rresp <= 2'b0;
                    end
                    default: begin
                        rdata <= 32'b0;
    ...
    logic [7:0] irq_sync0;  // Request synchroniser stage 0
    logic [7:0] irq_sync1;  // Request synchroniser stage 1
    logic [7:0] irq_sync2;  // Request synchroniser stage 2
    logic [7:0] irq_sync3;  // Request synchroniser stage 3
    logic [7:0] irq_hist;  // Previous settled request, for edge detection
    ...
    assign pending_value_set = irq_sync3 & (~irq_hist);
