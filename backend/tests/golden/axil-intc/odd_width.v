// SemiCraft v0.4.0
// Snippet: axil_intc (config hash: b56e99ff1770)
...
module axil_intc (
    ...
    input  wire [12:0] irq_in,     // Asynchronous interrupt requests
    output wire        irq_out     // Masked request; high while any enabled source pends
);

    reg [12:0] pending_value;  // PENDING.value (W1C) — current value
    wire [12:0] pending_value_set;  // PENDING.value (W1C) — set request; a set beats a same-cycle clear
    reg [12:0] enable_value;  // ENABLE.value (RW)
    wire [12:0] status_value;  // STATUS.value (RO) — sampled on read
    ...
    reg [12:0] irq_sync0;  // Request synchroniser stage 0
    reg [12:0] irq_sync1;  // Request synchroniser stage 1
    reg [12:0] irq_hist;  // Previous settled request, for edge detection
