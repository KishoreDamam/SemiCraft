// SemiCraft v0.4.0
// Snippet: axil_intc (config hash: 022d9334b4e7)
// AXI4-Lite interrupt controller, 1 source(s), level-triggered
//
...
module axil_intc (
    ...
    input  wire        irq_in,     // Asynchronous interrupt requests
    output wire        irq_out     // Masked request; high while any enabled source pends
);

    reg pending_value;  // PENDING.value (W1C) — current value
    wire pending_value_set;  // PENDING.value (W1C) — set request; a set beats a same-cycle clear
    reg enable_value;  // ENABLE.value (RW)
    wire status_value;  // STATUS.value (RO) — sampled on read
    ...
    reg irq_sync0;  // Request synchroniser stage 0
    reg irq_sync1;  // Request synchroniser stage 1
