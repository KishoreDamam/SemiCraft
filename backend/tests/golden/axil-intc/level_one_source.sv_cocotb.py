# SemiCraft v0.3.0
# cocotb testbench for axil_intc (config hash: 651b00fc2a79)
...
@cocotb.test(timeout_time=4720, timeout_unit="ns")
async def smoke(dut):
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 1, (
        f"SMOKE FAIL: bvalid at cycle 16 expected 1, "
        f"got {int(dut.bvalid.value)}"
    )
    assert dut.bresp.value == 0, (
        f"SMOKE FAIL: bresp at cycle 16 expected 0, "
        f"got {int(dut.bresp.value)}"
    )
    assert dut.irq_out.value == 0, (
        f"SMOKE FAIL: irq_out at cycle 16 expected 0, "
        f"got {int(dut.irq_out.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 0
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 0, (
        f"SMOKE FAIL: bvalid at cycle 17 expected 0, "
        f"got {int(dut.bvalid.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.arvalid.value = 0
    await Timer(1, units="ns")
    assert dut.rvalid.value == 1, (
        f"SMOKE FAIL: rvalid at cycle 18 expected 1, "
        f"got {int(dut.rvalid.value)}"
    )
    assert dut.rdata.value == 0, (
        f"SMOKE FAIL: rdata at cycle 18 expected 0, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
        f"SMOKE FAIL: rresp at cycle 18 expected 0, "
        f"got {int(dut.rresp.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 4
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 1, (
        f"SMOKE FAIL: bvalid at cycle 21 expected 1, "
        f"got {int(dut.bvalid.value)}"
    )
    assert dut.bresp.value == 0, (
        f"SMOKE FAIL: bresp at cycle 21 expected 0, "
        f"got {int(dut.bresp.value)}"
    )
    assert dut.irq_out.value == 0, (
        f"SMOKE FAIL: irq_out at cycle 21 expected 0, "
        f"got {int(dut.irq_out.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.irq_in.value = 1
    await Timer(1, units="ns")
    assert dut.bvalid.value == 0, (
        f"SMOKE FAIL: bvalid at cycle 22 expected 0, "
        f"got {int(dut.bvalid.value)}"
    )
    for _ in range(4):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.rvalid.value == 1, (
        f"SMOKE FAIL: rvalid at cycle 36 expected 1, "
        f"got {int(dut.rvalid.value)}"
    )
    assert dut.rdata.value == 1, (
        f"SMOKE FAIL: rdata at cycle 36 expected 1, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
        f"SMOKE FAIL: rresp at cycle 36 expected 0, "
        f"got {int(dut.rresp.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 4
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 1, (
        f"SMOKE FAIL: bvalid at cycle 39 expected 1, "
        f"got {int(dut.bvalid.value)}"
    )
    assert dut.bresp.value == 2, (
        f"SMOKE FAIL: bresp at cycle 39 expected 2, "
        f"got {int(dut.bresp.value)}"
    )
    assert dut.irq_out.value == 0, (
        f"SMOKE FAIL: irq_out at cycle 39 expected 0, "
        f"got {int(dut.irq_out.value)}"
    )
    await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.bvalid.value == 0, (
        f"SMOKE FAIL: bvalid at cycle 40 expected 0, "
        f"got {int(dut.bvalid.value)}"
    )
