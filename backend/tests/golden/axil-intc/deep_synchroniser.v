// SemiCraft v0.3.0
// Snippet: axil_intc (config hash: e44772d75fd5)
...
module axil_intc (
    ...
    input  wire [7:0]  irq_in,     // Asynchronous interrupt requests
    output wire        irq_out     // Masked request; high while any enabled source pends
);

    reg [7:0] pending_value;  // PENDING.value (W1C) — current value
    wire [7:0] pending_value_set;  // PENDING.value (W1C) — set request; a set beats a same-cycle clear
    reg [7:0] enable_value;  // ENABLE.value (RW)
    wire [7:0] status_value;  // STATUS.value (RO) — sampled on read
    ...
    reg [7:0] irq_sync0;  // Request synchroniser stage 0
    reg [7:0] irq_sync1;  // Request synchroniser stage 1
    reg [7:0] irq_sync2;  // Request synchroniser stage 2
    reg [7:0] irq_sync3;  // Request synchroniser stage 3
    reg [7:0] irq_hist;  // Previous settled request, for edge detection
