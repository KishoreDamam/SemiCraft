// SemiCraft v0.1.0
// Snippet: rr_arbiter (config hash: 2d639a698b31)
...
module rr_arbiter (
    ...
    input  logic [7:0] req,          // Request lines, one per requester
    output logic [7:0] grant,        // One-hot grant (zero when idle), registered
    output logic       grant_valid   // High when a grant is asserted (|grant)
);

    logic [2:0] ptr;  // Highest-priority requester index (rotates)
    logic [7:0] masked_req;  // Requests at or above the priority pointer
    logic [7:0] masked_gnt;  // Lowest-index grant within the masked window
    logic [7:0] unmasked_gnt;  // Lowest-index grant over all requests (wrap-around)
    logic [7:0] grant_nxt;  // Combinational rotate-priority grant decision
