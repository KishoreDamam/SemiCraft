# SemiCraft v0.4.0
# cocotb testbench for rr_arbiter (config hash: b56dc4a4ccc9)
...
async def smoke(dut):
    ...
    dut.p_req.value = 1
    await Timer(1, units="ns")
    assert dut.p_grantValid.value == 1, (
        f"SMOKE FAIL: p_grantValid at cycle 2 expected 1, "
        f"got {int(dut.p_grantValid.value)}"
    )
    assert dut.p_grant.value == 1, (
        f"SMOKE FAIL: p_grant at cycle 2 expected 1, "
        f"got {int(dut.p_grant.value)}"
    )
    await FallingEdge(dut.p_clk)
    dut.p_req.value = 15
    await FallingEdge(dut.p_clk)
    ...
    dut.p_req.value = 8
    await Timer(1, units="ns")
    assert dut.p_grantValid.value == 0, (
        f"SMOKE FAIL: p_grantValid at cycle 8 expected 0, "
        f"got {int(dut.p_grantValid.value)}"
    )
    await FallingEdge(dut.p_clk)
    dut.p_req.value = 8
