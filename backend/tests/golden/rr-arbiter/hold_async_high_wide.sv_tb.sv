// SemiCraft v0.4.0
// Testbench: rr_arbiter_tb (config hash: 44a0e0329fe0)
// Smoke testbench (stub, compile-checked only) for rr_arbiter
//
// Generated code is provided as-is, without warranty of any kind. Free for
// commercial and non-commercial use at the user's own risk.

`timescale 1ns/1ps

module rr_arbiter_tb;
    logic clk;
    logic rst;
    logic [15:0] req;
    logic [15:0] grant;
    logic grant_valid;

    // Free-running clock
    initial clk = 1'b0;
    always #5 clk = ~clk;

    // Device under test
    rr_arbiter dut (
        .clk         (clk),
        .rst         (rst),
        .req         (req),
        .grant       (grant),
        .grant_valid (grant_valid)
    );

    // Stimulus and self-checking assertions
    initial begin
        // Watchdog: fail loudly if the run hangs
        fork
            begin
                static int watchdog_i;
                for (watchdog_i = 0; watchdog_i < 224; watchdog_i++) @(posedge clk);
                $fatal(1, "TIMEOUT: rr_arbiter_tb exceeded 224 cycles");
            end
        join_none
        // Initialise inputs and assert reset
        req = 16'd0;
        rst = 1'd1;
        repeat (2) @(posedge clk);
        #1;
        rst = 1'd0;
        // Apply directed vectors; sample checks on the falling edge
        @(negedge clk);
        req = 16'd0;
        @(negedge clk);
        req = 16'd1;
        @(negedge clk);
        req = 16'd1;
        #1;
        if (grant_valid !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: grant_valid at cycle 2 expected 1, got %0d", grant_valid);
        end
        if (grant !== 16'd1) begin
            $fatal(1, "SMOKE FAIL: grant at cycle 2 expected 1, got %0d", grant);
        end
        @(negedge clk);
        req = 16'd65535;
        @(negedge clk);
        req = 16'd65535;
        @(negedge clk);
        req = 16'd65535;
        @(negedge clk);
        req = 16'd65535;
        @(negedge clk);
        req = 16'd0;
        @(negedge clk);
        req = 16'd32768;
        #1;
        if (grant_valid !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: grant_valid at cycle 8 expected 0, got %0d", grant_valid);
        end
        @(negedge clk);
        req = 16'd32768;
        $display("SMOKE PASS: rr_arbiter");
        $finish;
    end

    // Concurrent assertions (SVA)
    grant_onehot0: assert property (@(posedge clk) disable iff (rst) $onehot0(grant))
        else $fatal(1, "SVA FAIL: grant_onehot0");
    grant_reset_value: assert property (@(posedge clk) $fell(rst) |-> grant == 16'd0)
        else $fatal(1, "SVA FAIL: grant_reset_value");

endmodule
