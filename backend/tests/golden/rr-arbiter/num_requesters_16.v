// SemiCraft v0.1.0
// Snippet: rr_arbiter (config hash: 4efd3da86dce)
// 16-way round-robin arbiter, registered grant
//
...
module rr_arbiter (
    ...
    input  wire [15:0] req,          // Request lines, one per requester
    output reg  [15:0] grant,        // One-hot grant (zero when idle), registered
    output wire        grant_valid   // High when a grant is asserted (|grant)
);

    reg [3:0] ptr;  // Highest-priority requester index (rotates)
    wire [15:0] masked_req;  // Requests at or above the priority pointer
    wire [15:0] masked_gnt;  // Lowest-index grant within the masked window
    wire [15:0] unmasked_gnt;  // Lowest-index grant over all requests (wrap-around)
    wire [15:0] grant_nxt;  // Combinational rotate-priority grant decision
