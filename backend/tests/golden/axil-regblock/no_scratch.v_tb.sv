// SemiCraft v0.3.0
// Testbench: axil_regblock_tb (config hash: bebd91f6383f)
    ...
    initial begin
        ...
        fork
            begin
                ...
                for (watchdog_i = 0; watchdog_i < 344; watchdog_i++) @(posedge aclk);
                $fatal(1, "TIMEOUT: axil_regblock_tb exceeded 344 cycles");
            end
        join_none
        ...
        #1;
        if (rvalid !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: rvalid at cycle 19 expected 1, got %0d", rvalid);
        end
        if (rdata !== 32'd241) begin
            $fatal(1, "SMOKE FAIL: rdata at cycle 19 expected 241, got %0d", rdata);
        end
        if (rresp !== 2'd0) begin
            $fatal(1, "SMOKE FAIL: rresp at cycle 19 expected 0, got %0d", rresp);
        end
        @(negedge aclk);
        awaddr = 4'd12;
        awvalid = 1'd1;
        ...
        #1;
        if (bvalid !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: bvalid at cycle 22 expected 1, got %0d", bvalid);
        end
        if (bresp !== 2'd0) begin
            $fatal(1, "SMOKE FAIL: bresp at cycle 22 expected 0, got %0d", bresp);
        end
        if (cmd0_go !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: cmd0_go at cycle 22 expected 1, got %0d", cmd0_go);
        end
        if (cmd0_code !== 3'd7) begin
            $fatal(1, "SMOKE FAIL: cmd0_code at cycle 22 expected 7, got %0d", cmd0_code);
        end
        @(negedge aclk);
        araddr = 4'd12;
        arvalid = 1'd1;
        ...
        #1;
        if (bvalid !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: bvalid at cycle 23 expected 0, got %0d", bvalid);
        end
        @(negedge aclk);
        arvalid = 1'd0;
        #1;
        ...
        if (rdata !== 32'd0) begin
            $fatal(1, "SMOKE FAIL: rdata at cycle 24 expected 0, got %0d", rdata);
        end
        if (rresp !== 2'd0) begin
