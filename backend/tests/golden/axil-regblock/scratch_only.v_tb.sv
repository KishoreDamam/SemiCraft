// SemiCraft v0.4.0
// Testbench: axil_regblock_tb (config hash: 77eb4cb0dad0)
    ...
    initial begin
        ...
        fork
            begin
                ...
                for (watchdog_i = 0; watchdog_i < 272; watchdog_i++) @(posedge aclk);
                $fatal(1, "TIMEOUT: axil_regblock_tb exceeded 272 cycles");
            end
        join_none
        ...
        if (scratch_value !== 32'd4294967295) begin
            $fatal(1, "SMOKE FAIL: scratch_value at cycle 3 expected 4294967295, got %0d", scratch_value);
        end
        @(negedge aclk);
        araddr = 2'd0;
        arvalid = 1'd1;
        ...
        if (bresp !== 2'd0) begin
            $fatal(1, "SMOKE FAIL: bresp at cycle 10 expected 0, got %0d", bresp);
        end
        if (scratch_value !== 32'd2694947491) begin
            $fatal(1, "SMOKE FAIL: scratch_value at cycle 10 expected 2694947491, got %0d", scratch_value);
        end
        @(negedge aclk);
        awaddr = 2'd0;
        awvalid = 1'd1;
        ...
        if (scratch_value !== 32'd2690556508) begin
            $fatal(1, "SMOKE FAIL: scratch_value at cycle 13 expected 2690556508, got %0d", scratch_value);
        end
        @(negedge aclk);
        araddr = 2'd0;
        arvalid = 1'd1;
        ...
        if (rdata !== 32'd2690556508) begin
            $fatal(1, "SMOKE FAIL: rdata at cycle 15 expected 2690556508, got %0d", rdata);
        end
        if (rresp !== 2'd0) begin
