// SemiCraft v0.4.0
// Testbench: axil_regblock_tb (config hash: 239f6729f170)
...
module axil_regblock_tb;
    ...
    logic ctrl1_enable;
    logic [1:0] ctrl1_mode;
    logic [3:0] ctrl1_level;
    logic status0_busy;
    logic [3:0] status0_code;
    logic status1_busy;
    logic [3:0] status1_code;
    logic [3:0] irq0_flags;
    logic [3:0] irq0_flags_set;
    logic [3:0] irq1_flags;
    logic [3:0] irq1_flags_set;
    logic cmd0_go;
    logic [2:0] cmd0_code;
    logic cmd1_go;
    logic [2:0] cmd1_code;
    ...
    initial begin
        ...
        if (ctrl1_enable !== 1'd1) begin
            $fatal(1, "SMOKE FAIL: ctrl1_enable at cycle 0 expected 1, got %0d", ctrl1_enable);
        end
        if (ctrl1_mode !== 2'd0) begin
            $fatal(1, "SMOKE FAIL: ctrl1_mode at cycle 0 expected 0, got %0d", ctrl1_mode);
        end
        if (ctrl1_level !== 4'd0) begin
            $fatal(1, "SMOKE FAIL: ctrl1_level at cycle 0 expected 0, got %0d", ctrl1_level);
        end
        if (irq0_flags !== 4'd0) begin
            $fatal(1, "SMOKE FAIL: irq0_flags at cycle 0 expected 0, got %0d", irq0_flags);
        end
        if (irq1_flags !== 4'd0) begin
            $fatal(1, "SMOKE FAIL: irq1_flags at cycle 0 expected 0, got %0d", irq1_flags);
        end
        ...
        if (cmd1_go !== 1'd0) begin
            $fatal(1, "SMOKE FAIL: cmd1_go at cycle 0 expected 0, got %0d", cmd1_go);
        end
        if (cmd1_code !== 3'd0) begin
            $fatal(1, "SMOKE FAIL: cmd1_code at cycle 0 expected 0, got %0d", cmd1_code);
        end
        ...
        if (scratch_value !== 32'd2694947491) begin
            $fatal(1, "SMOKE FAIL: scratch_value at cycle 20 expected 2694947491, got %0d", scratch_value);
        end
        @(negedge aclk);
        awaddr = 6'd32;
        awvalid = 1'd1;
        ...
        if (scratch_value !== 32'd2690556508) begin
            $fatal(1, "SMOKE FAIL: scratch_value at cycle 23 expected 2690556508, got %0d", scratch_value);
        end
        @(negedge aclk);
        araddr = 6'd32;
        arvalid = 1'd1;
        ...
        if (rdata !== 32'd2690556508) begin
            $fatal(1, "SMOKE FAIL: rdata at cycle 25 expected 2690556508, got %0d", rdata);
        end
        if (rresp !== 2'd0) begin
