# SemiCraft v0.3.0
# cocotb testbench for axil_regblock (config hash: fc336c14f4f7)
...
async def smoke(dut):
    ...
    await Timer(1, units="ns")
    assert dut.rvalid.value == 1, (
        f"SMOKE FAIL: rvalid at cycle 19 expected 1, "
        f"got {int(dut.rvalid.value)}"
    )
    assert dut.rdata.value == 241, (
        f"SMOKE FAIL: rdata at cycle 19 expected 241, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
        f"SMOKE FAIL: rresp at cycle 19 expected 0, "
        f"got {int(dut.rresp.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 12
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 1, (
        f"SMOKE FAIL: bvalid at cycle 22 expected 1, "
        f"got {int(dut.bvalid.value)}"
    )
    assert dut.bresp.value == 0, (
        f"SMOKE FAIL: bresp at cycle 22 expected 0, "
        f"got {int(dut.bresp.value)}"
    )
    assert dut.cmd0_go.value == 1, (
        f"SMOKE FAIL: cmd0_go at cycle 22 expected 1, "
        f"got {int(dut.cmd0_go.value)}"
    )
    assert dut.cmd0_code.value == 7, (
        f"SMOKE FAIL: cmd0_code at cycle 22 expected 7, "
        f"got {int(dut.cmd0_code.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 12
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 0, (
        f"SMOKE FAIL: bvalid at cycle 23 expected 0, "
        f"got {int(dut.bvalid.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.arvalid.value = 0
    ...
    assert dut.rdata.value == 0, (
        f"SMOKE FAIL: rdata at cycle 24 expected 0, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
