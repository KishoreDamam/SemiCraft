# SemiCraft v0.3.0
# cocotb testbench for axil_regblock (config hash: 239f6729f170)
...
async def smoke(dut):
    ...
    dut.status1_busy.value = 0
    dut.status1_code.value = 0
    dut.irq0_flags_set.value = 0
    dut.irq1_flags_set.value = 0
    ...
    assert dut.ctrl1_enable.value == 1, (
        f"SMOKE FAIL: ctrl1_enable at cycle 0 expected 1, "
        f"got {int(dut.ctrl1_enable.value)}"
    )
    assert dut.ctrl1_mode.value == 0, (
        f"SMOKE FAIL: ctrl1_mode at cycle 0 expected 0, "
        f"got {int(dut.ctrl1_mode.value)}"
    )
    assert dut.ctrl1_level.value == 0, (
        f"SMOKE FAIL: ctrl1_level at cycle 0 expected 0, "
        f"got {int(dut.ctrl1_level.value)}"
    )
    assert dut.irq0_flags.value == 0, (
        f"SMOKE FAIL: irq0_flags at cycle 0 expected 0, "
        f"got {int(dut.irq0_flags.value)}"
    )
    assert dut.irq1_flags.value == 0, (
        f"SMOKE FAIL: irq1_flags at cycle 0 expected 0, "
        f"got {int(dut.irq1_flags.value)}"
    )
    ...
    assert dut.cmd1_go.value == 0, (
        f"SMOKE FAIL: cmd1_go at cycle 0 expected 0, "
        f"got {int(dut.cmd1_go.value)}"
    )
    assert dut.cmd1_code.value == 0, (
        f"SMOKE FAIL: cmd1_code at cycle 0 expected 0, "
        f"got {int(dut.cmd1_code.value)}"
    )
    ...
    assert dut.scratch_value.value == 2694947491, (
        f"SMOKE FAIL: scratch_value at cycle 20 expected 2694947491, "
        f"got {int(dut.scratch_value.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 32
    ...
    assert dut.scratch_value.value == 2690556508, (
        f"SMOKE FAIL: scratch_value at cycle 23 expected 2690556508, "
        f"got {int(dut.scratch_value.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 32
    ...
    assert dut.rdata.value == 2690556508, (
        f"SMOKE FAIL: rdata at cycle 25 expected 2690556508, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
