...
module axil_regblock (
    ...
    output logic        ctrl1_enable,     // CTRL1.enable (RW)
    output logic [1:0]  ctrl1_mode,       // CTRL1.mode (RW)
    output logic [3:0]  ctrl1_level,      // CTRL1.level (RW)
    input  logic        status0_busy,     // STATUS0.busy (RO) — sampled on read
    input  logic [3:0]  status0_code,     // STATUS0.code (RO) — sampled on read
    input  logic        status1_busy,     // STATUS1.busy (RO) — sampled on read
    input  logic [3:0]  status1_code,     // STATUS1.code (RO) — sampled on read
    ...
    output logic [3:0]  irq1_flags,       // IRQ1.flags (W1C) — current value
    input  logic [3:0]  irq1_flags_set,   // IRQ1.flags (W1C) — set request; a set beats a same-cycle clear
    output logic        cmd0_go,          // CMD0.go (WO) — reads back as 0
    output logic [2:0]  cmd0_code,        // CMD0.code (WO) — reads back as 0
    output logic        cmd1_go,          // CMD1.go (WO) — reads back as 0
    output logic [2:0]  cmd1_code,        // CMD1.code (WO) — reads back as 0
    ...
    logic wr_addr_ctrl1;  // Write address selects CTRL1
    ...
    logic wr_addr_status1;  // Write address selects STATUS1
    ...
    logic wr_addr_irq1;  // Write address selects IRQ1
    ...
    logic wr_addr_cmd1;  // Write address selects CMD1
    ...
    logic wr_sel_ctrl1;  // Write updates CTRL1
    ...
    logic wr_sel_irq1;  // Write updates IRQ1
    ...
    logic wr_sel_cmd1;  // Write updates CMD1
