// SemiCraft v0.4.0
// Snippet: axil_uart (config hash: 8ae9b5ab5c25)
...
module axil_uart (
    ...
    input  wire        uart_rx,    // Serial input, asynchronous; idles high
    output reg         uart_tx     // Serial output; idles high
);

    wire status_tx_busy;  // STATUS.tx_busy (RO) — sampled on read
    ...
    reg status_rx_overrun;  // STATUS.rx_overrun (W1C) — current value
    reg status_rx_overrun_set;  // STATUS.rx_overrun (W1C) — set request; a set beats a same-cycle clear
    reg status_frame_error;  // STATUS.frame_error (W1C) — current value
    reg status_frame_error_set;  // STATUS.frame_error (W1C) — set request; a set beats a same-cycle clear
    reg [7:0] txdata_data;  // TXDATA.data (WO) — reads back as 0
    reg [7:0] rxdata_data;  // RXDATA.data (RO) — sampled on read
    reg [15:0] bauddiv_div;  // BAUDDIV.div (RW)
    ...
    wire wr_addr_bauddiv;  // Write address selects BAUDDIV
    ...
    wire wr_sel_bauddiv;  // Write updates BAUDDIV
    ...
    reg rx_sync0;  // rx synchroniser stage 0
    reg rx_sync1;  // rx synchroniser stage 1
    reg rx_sync2;  // rx synchroniser stage 2
    reg rx_sync3;  // rx synchroniser stage 3
    reg [1:0] tx_state;  // Transmit FSM: 0 IDLE, 1 START, 2 DATA, 3 STOP
    reg [15:0] tx_cnt;  // Transmit baud counter
    reg [7:0] tx_shift;  // Transmit shift register, empties LSB first
    reg [2:0] tx_bit;  // Transmit bit index
    reg tx_go;  // TXDATA write strobe, delayed one cycle
    reg [1:0] rx_state;  // Receive FSM: 0 IDLE, 1 START, 2 DATA, 3 STOP
    reg [15:0] rx_cnt;  // Receive baud counter
    reg [7:0] rx_shift;  // Receive shift register, fills from the MSB
    reg [2:0] rx_bit;  // Receive bit index
            ...
            case (rx_state)
                ...
                2'b11: begin
                    ...
                    if (rx_cnt == (bauddiv_div - 16'b1)) begin
                        ...
                        status_frame_error_set <= !rx_sync3;
                    end
                end
