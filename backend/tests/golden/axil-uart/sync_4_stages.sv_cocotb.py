# SemiCraft v0.4.0
# cocotb testbench for axil_uart (config hash: b6ff7eedafa2)
...
@cocotb.test(timeout_time=9840, timeout_unit="ns")
async def smoke(dut):
    ...
    assert dut.rdata.value == 4, (
        f"SMOKE FAIL: rdata at cycle 2 expected 4, "
    ...
    await Timer(1, units="ns")
    assert dut.uart_tx.value == 0, (
        f"SMOKE FAIL: uart_tx at cycle 27 expected 0, "
        f"got {int(dut.uart_tx.value)}"
    )
    for _ in range(4):
        await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.uart_tx.value == 0, (
        f"SMOKE FAIL: uart_tx at cycle 31 expected 0, "
        f"got {int(dut.uart_tx.value)}"
    )
    for _ in range(4):
        await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.uart_tx.value == 1, (
        f"SMOKE FAIL: uart_tx at cycle 35 expected 1, "
        f"got {int(dut.uart_tx.value)}"
    )
    for _ in range(4):
        await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.uart_tx.value == 0, (
        f"SMOKE FAIL: uart_tx at cycle 39 expected 0, "
        f"got {int(dut.uart_tx.value)}"
    )
    for _ in range(4):
        await FallingEdge(dut.aclk)
    await Timer(1, units="ns")
    assert dut.uart_tx.value == 1, (
        f"SMOKE FAIL: uart_tx at cycle 43 expected 1, "
        f"got {int(dut.uart_tx.value)}"
    )
    for _ in range(3):
        await FallingEdge(dut.aclk)
    ...
    await Timer(1, units="ns")
    assert dut.uart_tx.value == 1, (
        f"SMOKE FAIL: uart_tx at cycle 46 expected 1, "
        f"got {int(dut.uart_tx.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.arvalid.value = 0
    ...
    assert dut.rdata.value == 0, (
        f"SMOKE FAIL: rdata at cycle 47 expected 0, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
    ...
    await Timer(1, units="ns")
    assert dut.rvalid.value == 1, (
        f"SMOKE FAIL: rvalid at cycle 97 expected 1, "
        f"got {int(dut.rvalid.value)}"
    )
    assert dut.rdata.value == 2, (
        f"SMOKE FAIL: rdata at cycle 97 expected 2, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
        f"SMOKE FAIL: rresp at cycle 97 expected 0, "
        f"got {int(dut.rresp.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 8
    ...
    await Timer(1, units="ns")
    assert dut.rvalid.value == 1, (
        f"SMOKE FAIL: rvalid at cycle 99 expected 1, "
        f"got {int(dut.rvalid.value)}"
    )
    assert dut.rdata.value == 45, (
        f"SMOKE FAIL: rdata at cycle 99 expected 45, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
        f"SMOKE FAIL: rresp at cycle 99 expected 0, "
        f"got {int(dut.rresp.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.awaddr.value = 0
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 1, (
        f"SMOKE FAIL: bvalid at cycle 102 expected 1, "
        f"got {int(dut.bvalid.value)}"
    )
    assert dut.bresp.value == 0, (
        f"SMOKE FAIL: bresp at cycle 102 expected 0, "
        f"got {int(dut.bresp.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.araddr.value = 0
    ...
    await Timer(1, units="ns")
    assert dut.bvalid.value == 0, (
        f"SMOKE FAIL: bvalid at cycle 103 expected 0, "
        f"got {int(dut.bvalid.value)}"
    )
    await FallingEdge(dut.aclk)
    dut.arvalid.value = 0
    await Timer(1, units="ns")
    assert dut.rvalid.value == 1, (
        f"SMOKE FAIL: rvalid at cycle 104 expected 1, "
        f"got {int(dut.rvalid.value)}"
    )
    assert dut.rdata.value == 0, (
        f"SMOKE FAIL: rdata at cycle 104 expected 0, "
        f"got {int(dut.rdata.value)}"
    )
    assert dut.rresp.value == 0, (
        f"SMOKE FAIL: rresp at cycle 104 expected 0, "
        f"got {int(dut.rresp.value)}"
    )
